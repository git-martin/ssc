...
namespace SSCWeb.App_Code
{
    ...
    public class SqLiteHelper
    {
        ...
        {
            return null == bllInstance ? new SqLiteHelper(GlobalConstants.SqLiteConnectString) : bllInstance;
        }
        ...
        /// 生成单式上传订单专用
        /// </summary>
        /// <param name="sqlTicket"></param>
        /// <param name="ticketParasList"></param>
        /// <returns></returns>
        public int ExecuteSqlParasTran02(string sqlTicket, List<SQLiteParameter[]> ticketParasList, string sqlOrder, SQLiteParameter[] orderParas)
        {
            int n = 0;
            using (SQLiteConnection conn = new SQLiteConnection(this.connectionString))
            {
                ...
                {
                    //票
                    cmd.CommandText = sqlTicket;
                    for (n = 0; n < ticketParasList.Count; n++)
                    {
                        if (ticketParasList[n] != null)
                        {
                            cmd.Parameters.AddRange(ticketParasList[n]);
                            cmd.ExecuteNonQuery();
                        }
                    }
                    ...
                    cmd.Parameters.AddRange(orderParas);
                    cmd.ExecuteNonQuery();
