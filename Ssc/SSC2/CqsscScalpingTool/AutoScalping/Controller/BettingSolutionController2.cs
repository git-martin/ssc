...
{
    public class BettingSolutionController2: BettingSolutionControllerBase
    {
        ...
        private static List<List<int>> RandomSpliteNumbers(int seperateJiangCount, int seperateQcwCount)
        {
            ...
            Seperated(ref result,ref groupJ,seperateJiangCount,false);
            Seperated(ref result, ref groupQ, seperateQcwCount,true);
            return result;
        }

        private static void Seperated(ref List<List<int>> result, ref List<int> numbers, int dividedCount,bool isQcw)
        {
            if (dividedCount == 1)
            {
                result.Add(numbers);
            ...
            else if (dividedCount == 3)
            {
                ...
                if (isQcw)
                {
                    result.Add(tLst1.Union(tLst2).ToList<int>());
                    result.Add(tLst1.Union(tLst3).ToList<int>());
                    result.Add(tLst2.Union(tLst3).ToList<int>());
                }
                else
                {
                    result.Add(tLst1);
